...
describe("the JavaScript language", function(){
  ...
  describe("considers functions as first class citizens so that", function(){
    ...
    it("may return arrays that contains functions and so on", function(){
        function example(){
           ...
           return [function(number) {return [0,10]}];
        }
    ...
    it("may return arrays that contains closures and so on", function(){
        function example(){
           // write the missing code here
           var a = 9;
           return [function(number) { return [0, a + number]; }];
        }
  ...
  describe("has multiple ways to define and create objects", function(){
    ...
    it("can create methods dynamically on an object instance", function(){
        ...
        for (var i = 0; i < methodNames.length; i++){
          obj[methodNames[i]] = function(){ return 'it works ' + i;};
        }
        expect(obj.meow()).toEqual('it works 2');
    });
  ...
  describe("commons patterns with functions and behaviors", function(){
    ...
    it("hoists variables the way you probably dont expect", function(){
      // El for no tiene su propio scope por lo que la variable que coge la funcion
      // adquiere el valor por la referencia al no pasarlesela por parametro y copiarla
      // se soluciona pasandole la variable por parametro o creando una funcion autoejecutable
  ...
  context("has ways to simulate classes", function(){
      ...
      context("and the THIS keyword", function(){
        ...
        it("works different on dettached functions", function(){
          window.kilos = 10;
          var feed = cat.feed; // Al estar definiendo la funcion con una asignacion
          //esta heredando el scope global al no haberle definido que pertenece al
          //scope que se esta heredando this.feed = cat.feed
        ...
        it("works different when function is attached to other object", function(){
            ...
            otherCat.feed();
            expect(otherCat.kilos).toEqual(3);
            expect(cat.kilos).toEqual(1);
        });
